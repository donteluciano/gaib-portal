...
export default function PoliticalTab({ siteId }: Props) {
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px' }}><p style={{ color: '#6B7280' }}>Loading political data...</p></div>;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div><h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', fontFamily: 'Georgia, serif' }}>Officials & Stakeholders</h2><p style={{ color: '#6B7280', fontSize: '14px' }}>{data.officials.length} contacts tracked</p></div>
          <button onClick={() => setShowAddOfficial(true)} style={{ padding: '10px 16px', backgroundColor: '#2563EB', color: '#FFFFFF', fontWeight: 500, borderRadius: '8px', border: 'none', cursor: 'pointer' }}>+ Add Contact</button>
        </div>
        {showAddOfficial && (
          <div style={{ marginBottom: '24px', padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={addOfficial} disabled={!newOfficial.name} style={{ padding: '10px 16px', backgroundColor: '#2563EB', color: '#FFFFFF', fontWeight: 500, borderRadius: '8px', border: 'none', cursor: 'pointer', opacity: !newOfficial.name ? 0.5 : 1 }}>Add Contact</button>
              <button onClick={() => setShowAddOfficial(false)} style={{ padding: '10px 16px', backgroundColor: '#F3F4F6', color: '#374151', borderRadius: '8px', border: '1px solid #D1D5DB', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )}
        {data.officials.length === 0 ? <div style={{ textAlign: 'center', padding: '32px', color: '#6B7280' }}><p>No officials tracked yet. Add contacts to track political relationships.</p></div> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {data.officials.map(official => (
              <div key={official.id} style={{ padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{ flex: 1 }}><p style={{ color: '#111827', fontWeight: 500 }}>{official.name}</p><p style={{ color: '#6B7280', fontSize: '14px' }}>{official.title} {official.contact && `• ${official.contact}`}</p></div>
                {official.lastContact && <span style={{ color: '#6B7280', fontSize: '14px' }}>Last: {official.lastContact}</span>}
                ...
                <button onClick={() => removeOfficial(official.id)} style={{ color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div><h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', fontFamily: 'Georgia, serif' }}>Political Risk Matrix</h2><p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>Score each factor 1-5 based on site conditions</p></div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {politicalFactors.map((factor) => (
            <div key={factor.key} style={{ padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><p style={{ color: '#111827', fontWeight: 500 }}>{factor.label}</p><span style={{ color: '#9CA3AF', fontSize: '12px' }}>Weight: {factor.weight}%</span></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>{[1, 2, 3, 4, 5].map((score) => <button key={score} onClick={() => updateData('politicalScores', { ...data.politicalScores, [factor.key]: score })} style={{ width: '32px', height: '32px', borderRadius: '50%', fontSize: '14px', fontWeight: 500, border: 'none', cursor: 'pointer', backgroundColor: data.politicalScores[factor.key] === score ? (score <= 2 ? '#EF4444' : score <= 3 ? '#EAB308' : '#22C55E') : '#E5E7EB', color: data.politicalScores[factor.key] === score ? '#FFFFFF' : '#6B7280' }}>{score}</button>)}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div><h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', fontFamily: 'Georgia, serif' }}>Community Sentiment Scorecard</h2><p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>Assess community acceptance factors</p></div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {communityFactors.map((factor) => (
            <div key={factor.key} style={{ padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><p style={{ color: '#111827', fontWeight: 500 }}>{factor.label}</p><span style={{ color: '#9CA3AF', fontSize: '12px' }}>Weight: {factor.weight}%</span></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>{[1, 2, 3, 4, 5].map((score) => <button key={score} onClick={() => updateData('communityScores', { ...data.communityScores, [factor.key]: score })} style={{ width: '32px', height: '32px', borderRadius: '50%', fontSize: '14px', fontWeight: 500, border: 'none', cursor: 'pointer', backgroundColor: data.communityScores[factor.key] === score ? (score <= 2 ? '#EF4444' : score <= 3 ? '#EAB308' : '#22C55E') : '#E5E7EB', color: data.communityScores[factor.key] === score ? '#FFFFFF' : '#6B7280' }}>{score}</button>)}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div><h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', fontFamily: 'Georgia, serif' }}>Community Benefit Agreement Builder</h2><p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>Toggle offerings and estimate costs</p></div>
          <div style={{ textAlign: 'right' }}><p style={{ fontSize: '28px', fontWeight: 700, color: '#2563EB' }}>${(cbaTotal / 1000).toFixed(0)}K</p><p style={{ color: '#6B7280', fontSize: '14px' }}>Est. CBA Cost</p></div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {cbaOptions.map((option) => (
            <div key={option.key} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px' }}>
              <button onClick={() => updateData('cbaItems', { ...data.cbaItems, [option.key]: { ...data.cbaItems[option.key], enabled: !data.cbaItems[option.key]?.enabled } })} style={{ width: '24px', height: '24px', borderRadius: '4px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backgroundColor: data.cbaItems[option.key]?.enabled ? '#2563EB' : '#FFFFFF', border: data.cbaItems[option.key]?.enabled ? 'none' : '2px solid #D1D5DB' }}>
              ...
              <div style={{ flex: 1 }}><p style={{ color: '#111827', fontWeight: 500 }}>{option.label}</p><p style={{ color: '#6B7280', fontSize: '14px' }}>{option.description}</p></div>
      ...
      {/* Notes */}
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', marginBottom: '16px', fontFamily: 'Georgia, serif' }}>Political Notes</h2>
