...
export default function DocumentsTab({ siteId }: Props) {
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px' }}><p style={{ color: '#6B7280' }}>Loading documents...</p></div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: 600, color: '#111827', fontFamily: 'Georgia, serif' }}>Documents & Files</h2>
          <p style={{ color: '#6B7280', fontSize: '14px' }}>{docs.length} document{docs.length !== 1 ? 's' : ''}</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} style={{ padding: '10px 16px', backgroundColor: '#2563EB', color: '#FFFFFF', fontWeight: 500, borderRadius: '8px', border: 'none', cursor: 'pointer' }}>+ Add Document</button>
      </div>

      {showForm && (
        <div style={{ padding: '20px', backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px' }}>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
            <button type="button" onClick={() => setAddMode('upload')} style={{ flex: 1, padding: '10px 16px', borderRadius: '8px', fontWeight: 500, backgroundColor: addMode === 'upload' ? '#2563EB' : '#F3F4F6', color: addMode === 'upload' ? '#FFFFFF' : '#374151', border: 'none', cursor: 'pointer' }}>📤 Upload File</button>
            <button type="button" onClick={() => setAddMode('link')} style={{ flex: 1, padding: '10px 16px', borderRadius: '8px', fontWeight: 500, backgroundColor: addMode === 'link' ? '#2563EB' : '#F3F4F6', color: addMode === 'link' ? '#FFFFFF' : '#374151', border: 'none', cursor: 'pointer' }}>🔗 Add Link</button>
          </div>
          ...
          {addMode === 'upload' ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div onClick={() => fileInputRef.current?.click()} style={{ border: '2px dashed', borderRadius: '8px', padding: '32px', textAlign: 'center', cursor: 'pointer', borderColor: selectedFile ? '#22C55E' : '#D1D5DB', backgroundColor: selectedFile ? '#F0FDF4' : '#F9FAFB' }}>
                {selectedFile ? (<div><div style={{ fontSize: '32px', marginBottom: '8px' }}>{getFileIcon(selectedFile.type)}</div><p style={{ color: '#111827', fontWeight: 500 }}>{selectedFile.name}</p><p style={{ color: '#6B7280', fontSize: '14px' }}>{formatFileSize(selectedFile.size)}</p><button type="button" onClick={(e) => { e.stopPropagation(); setSelectedFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} style={{ marginTop: '8px', color: '#DC2626', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px' }}>Remove</button></div>) : (<div><div style={{ fontSize: '32px', marginBottom: '8px' }}>📁</div><p style={{ color: '#111827', fontWeight: 500 }}>Click to select a file</p><p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>PDF, Word, Excel, Images • Max {MAX_FILE_SIZE / 1024 / 1024}MB</p></div>)}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '6px', fontWeight: 500 }}>Document Name *</label><input type="text" value={newDoc.name} onChange={(e) => setNewDoc({ ...newDoc, name: e.target.value })} placeholder="Phase I ESA Report" style={inputStyle} /></div>
                <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '6px', fontWeight: 500 }}>Category</label><select value={newDoc.category} onChange={(e) => setNewDoc({ ...newDoc, category: e.target.value as DocCategory })} style={inputStyle}>{Object.entries(categoryLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
              </div>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
              <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '6px', fontWeight: 500 }}>Document Name *</label><input type="text" value={newDoc.name} onChange={(e) => setNewDoc({ ...newDoc, name: e.target.value })} placeholder="Phase I ESA Report" style={inputStyle} /></div>
              <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '6px', fontWeight: 500 }}>URL *</label><input type="url" value={newDoc.url} onChange={(e) => setNewDoc({ ...newDoc, url: e.target.value })} placeholder="https://drive.google.com/..." style={inputStyle} /></div>
              <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '6px', fontWeight: 500 }}>Category</label><select value={newDoc.category} onChange={(e) => setNewDoc({ ...newDoc, category: e.target.value as DocCategory })} style={inputStyle}>{Object.entries(categoryLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
            </div>
          )}
          <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
            <button onClick={addDoc} disabled={saving} style={{ padding: '10px 20px', backgroundColor: '#2563EB', color: '#FFFFFF', fontWeight: 500, borderRadius: '8px', border: 'none', cursor: 'pointer', opacity: saving ? 0.5 : 1 }}>{saving ? (addMode === 'upload' ? 'Uploading...' : 'Adding...') : (addMode === 'upload' ? 'Upload' : 'Add Link')}</button>
            <button onClick={resetForm} style={{ padding: '10px 20px', backgroundColor: '#F3F4F6', color: '#374151', borderRadius: '8px', border: '1px solid #D1D5DB', cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden' }}>
        {docs.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center' }}><div style={{ fontSize: '32px', marginBottom: '16px' }}>📄</div><p style={{ color: '#111827', fontWeight: 500, marginBottom: '8px' }}>No documents yet</p><p style={{ color: '#6B7280', fontSize: '14px' }}>Upload files or add links to Google Drive, Dropbox, etc.</p></div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1px solid #E5E7EB', backgroundColor: '#F9FAFB' }}><th style={{ padding: '12px 24px', textAlign: 'left', color: '#374151', fontWeight: 500, fontSize: '14px' }}>Name</th><th style={{ padding: '12px 24px', textAlign: 'left', color: '#374151', fontWeight: 500, fontSize: '14px' }}>Category</th><th style={{ padding: '12px 24px', textAlign: 'left', color: '#374151', fontWeight: 500, fontSize: '14px' }}>Size</th><th style={{ padding: '12px 24px', textAlign: 'left', color: '#374151', fontWeight: 500, fontSize: '14px' }}>Added</th><th style={{ padding: '12px 24px', textAlign: 'right', color: '#374151', fontWeight: 500, fontSize: '14px' }}>Actions</th></tr></thead>
            <tbody>
              {docs.map((doc) => (
                <tr key={doc.id} style={{ borderBottom: '1px solid #E5E7EB' }}>
                  <td style={{ padding: '12px 24px' }}><a href={doc.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#111827', textDecoration: 'none' }}><span>{getFileIcon(doc.file_type, doc.url)}</span><span>{doc.name}</span><span style={{ color: '#9CA3AF' }}>↗</span></a></td>
                  ...
                  <td style={{ padding: '12px 24px', color: '#6B7280', fontSize: '14px' }}>{formatFileSize(doc.file_size) || '—'}</td>
                  <td style={{ padding: '12px 24px', color: '#6B7280' }}>{doc.date_added}</td>
                  <td style={{ padding: '12px 24px', textAlign: 'right' }}><button onClick={() => removeDoc(doc.id, doc.url)} style={{ color: '#6B7280', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px' }}>Remove</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p style={{ color: '#6B7280', fontSize: '14px', textAlign: 'center' }}>💡 Upload files directly or link to external storage like Google Drive or Dropbox.</p>
    </div>
  );
}
