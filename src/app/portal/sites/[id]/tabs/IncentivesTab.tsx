...
export default function IncentivesTab({ siteId }: Props) {
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px' }}><p style={{ color: '#6B7280' }}>Loading incentives...</p></div>;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
        <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
          <p style={{ color: '#6B7280', fontSize: '14px' }}>Total Estimated Value</p>
          <p style={{ fontSize: '28px', fontWeight: 700, color: '#2563EB', marginTop: '8px' }}>${(totalEstimatedValue / 1000000).toFixed(1)}M</p>
          <p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>Across all incentives {saving && <span style={{ color: '#2563EB' }}>• Saving...</span>}</p>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
          <p style={{ color: '#6B7280', fontSize: '14px' }}>Incentives Agreed</p>
          <p style={{ fontSize: '28px', fontWeight: 700, color: '#22C55E', marginTop: '8px' }}>{agreedIncentives}</p>
          <p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>of {incentiveTypes.length} types</p>
        </div>
        <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
          <p style={{ color: '#6B7280', fontSize: '14px' }}>In Discussion</p>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', marginTop: '4px' }}>Active negotiations</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {incentiveTypes.map((type) => (
          <div key={type.key} style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden' }}>
            <button onClick={() => setExpandedType(expandedType === type.key ? null : type.key)} style={{ width: '100%', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'none', border: 'none', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <h3 style={{ fontSize: '18px', fontWeight: 500, color: '#111827' }}>{type.name}</h3>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                {(incentives[type.key]?.estimatedValue || 0) > 0 && <span style={{ color: '#2563EB', fontWeight: 500 }}>${((incentives[type.key]?.estimatedValue || 0) / 1000000).toFixed(1)}M</span>}
                <svg style={{ width: '20px', height: '20px', color: '#6B7280', transform: expandedType === type.key ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
              </div>
            </button>
            {expandedType === type.key && (
              <div style={{ padding: '16px', borderTop: '1px solid #E5E7EB', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', padding: '16px', backgroundColor: '#F9FAFB', borderRadius: '8px' }}>
                  <div><p style={{ color: '#6B7280', fontSize: '12px' }}>Typical Value</p><p style={{ color: '#111827', fontSize: '14px' }}>{type.typicalValue}</p></div>
                  <div><p style={{ color: '#6B7280', fontSize: '12px' }}>Description</p><p style={{ color: '#111827', fontSize: '14px' }}>{type.description}</p></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '8px' }}>Status</label><select value={incentives[type.key]?.status || 'not_explored'} onChange={(e) => updateIncentive(type.key, 'status', e.target.value)} style={inputStyle}><option value="not_explored">Not Explored</option><option value="in_discussion">In Discussion</option><option value="agreed">Agreed</option><option value="denied">Denied</option></select></div>
                  <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '8px' }}>Estimated Value ($)</label><input type="number" min="0" value={incentives[type.key]?.estimatedValue || 0} onChange={(e) => updateIncentive(type.key, 'estimatedValue', Math.max(0, parseInt(e.target.value) || 0))} style={inputStyle} placeholder="0" /></div>
                </div>
                <div><label style={{ display: 'block', fontSize: '14px', color: '#374151', marginBottom: '8px' }}>Notes (Contact, Terms, Negotiations)</label><textarea value={incentives[type.key]?.notes || ''} onChange={(e) => updateIncentive(type.key, 'notes', e.target.value)} rows={2} style={{ ...inputStyle, resize: 'none' }} placeholder="Contact name, terms, conditions, negotiation notes..." /></div>
              </div>
            )}
          </div>
        ))}
      ...
      {totalEstimatedValue > 0 && (
        <div style={{ backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: '12px', padding: '24px' }}>
          <h3 style={{ fontSize: '20px', fontWeight: 600, color: '#2563EB', marginBottom: '16px', fontFamily: 'Georgia, serif' }}>Exit Pitch Summary</h3>
          <p style={{ color: '#111827' }}>This site comes with an estimated <span style={{ color: '#2563EB', fontWeight: 700 }}>${(totalEstimatedValue / 1000000).toFixed(1)}M</span> in municipal incentives, including:</p>
          <ul style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {incentiveTypes.filter(t => incentives[t.key]?.status === 'agreed' || (incentives[t.key]?.estimatedValue || 0) > 0).map(type => (
              <li key={type.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#6B7280' }}>
